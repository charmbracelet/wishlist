...
func init() {
	rootCmd.PersistentFlags().StringVarP(&configFile, "config", "c", "", "Path to the config file to use. Defaults to, in order of preference: $PWD/.wishlist/config.yaml, $PWD/.wishlist/config.yml, $HOME/.ssh/config.yaml, $HOME/.ssh/config.yml, $HOME/.ssh/config, /etc/ssh/ssh_config")
